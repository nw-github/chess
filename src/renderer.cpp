...
namespace zc
{
    ...
    void BoardRenderer::Render(sf::RenderTarget &target)
    {
        ...
        for (Int y = 0; y < Board::SIZE; y++)
        {
            for (Int x = 0; x < Board::SIZE; x++)
            {
                ...
                if (mBoard.IsValid(mSelected) && mBoard.IsValidMove(mSelected, position))
                    square.setFillColor({square.getFillColor().r, square.getFillColor().g, square.getFillColor().b, 180});
        ...
        if (mBoard.IsValid(mSelected))
        {
            ...
            auto &sprite = mSprites[piece.team][piece.type];
            sprite.setPosition(GetMousePosition(target) - sf::Vector2f(PIECE_SIZE / 2.f, PIECE_SIZE / 2.f));
            mPieces.draw(sprite);
        }
    ...
    void BoardRenderer::RenderPromotionDialog(sf::RenderTarget &target)
    {
        ...
        sf::RectangleShape background(sf::Vector2f{(PIECE_SIZE + PADDING * 2), headerY + (PIECE_SIZE + PADDING) * promotions.size() + PADDING});
        background.setFillColor(sf::Color{70, 70, 70});
        background.setPosition(sf::Vector2f{mBackground.getSize()} / 2.f - background.getSize() / 2.f);

        header.setPosition(sf::Vector2f{
            (background.getPosition().x + background.getSize().x / 2.f) - header.getLocalBounds().width / 2.f,
        ...
        for (const auto &piece : promotions)
        {
            ...
            sprite.setPosition(sf::Vector2f{background.getPosition().x + PADDING, background.getPosition().y + y});
    ...
    std::optional<Vector> BoardRenderer::ScreenToCoords(sf::RenderTarget &target) const
    {
        ...
        const auto result = Vector{static_cast<Int>(mouse.x / PIECE_SIZE), static_cast<Int>(mouse.y / PIECE_SIZE)};
